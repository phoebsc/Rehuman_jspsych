...
function uploadDataToServer(data) {
  const participantID = getUrlParam();
  const audioRef = ref(storage, `audio/${participantID}`);
  const eegRef = ref(storage, `eeg/${participantID}.csv`);

  // Audio data
  // data.trials.find(({trial-type})=>trial_type === "html-audio-response").response

  /*
  uploadBytes(audioRef, data.audio_file);
  uploadBytes(eegRef, data.muse_recording);
  */

  fetch(data.audio_url)
    .then((response) => response.blob())
    .then((blob) => uploadBytes(audioRef, blob));

  fetch(data.muse_url)
    .then((response) => response.blob())
    .then((blob) => uploadBytes(eegRef, blob));
...
var instruction_trial = {
  ...
  pages: [
    ...
    "Be sure to have the Muse headband readily available for this experiment.",
    "While we recording the data for this first stage, think about ...",
  ],
...
var recording_block = {
  ...
  instructions: "Connect your Muse headband using Bluetooth.",
  recording_id: "test",
